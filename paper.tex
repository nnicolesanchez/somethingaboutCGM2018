...
\title{Not So Heavy Metals: Black Hole Transport of OVI in the Circumgalactic Medium of ChaNGA Simulations}
...
\textbf{[UNDER CONSTRUCTION]} We examine the cosmological hydrodynamic simulation ROMULUS25 (Tremmel et al. 2017) and a set of zoom-in ``genetically modified'' Milky Way-mass galaxies to study the effects of galaxy evolution and SMBH feedback on the CGM. We compare the column densities of OVI in the Milky Way-mass galaxies of ROMULUS25 and compare them with observations from the COS-Halos Survey. We determine that a galaxy's star formation history and accretion rate have little effect on the appearance of OVI in its CGM while column densities of OVI are more likely tied to galaxy halo mass. The set of zoom-in, genetically modified Milky Way-mass galaxies further confirm this result and further examine the effect of AGN feedback on the CGM's OVI. We find that the SMBH acts as the physical mechanism transporting metals out into its host halo thereby significantly impacting the appearance of OVI found in the CGM.
...
\begin{figure}[]
\centerline{\resizebox{1.02\hsize}{!}{\includegraphics[angle=0]{ROMhistogram}}}
...
\begin{table*}[ht!] % `*' makes it go across both columns instead of one
\caption{Zoom-In Galaxies Properties \textit{with BHs} at z = 0.17} % title of Table
\centering % used for centering table
...
%heading
\hline % inserts single horizontal line
P0 & 1.08 $\times$ 10$^{12}$ & 1.09 $\times$ 10$^{11}$ & 5.71 $\times$ 10$^{10}$ & 1.02 $\times$ 10$^{11}$ & 268.9 & 5.12 $\times$ 10$^5$ & \textbf{7.34 $\times$ 10$^{10}$}\\ % inserting body of the table
GM1 & 1.07 $\times$ 10$^{12}$ & 1.01 $\times$ 10$^{11}$ & 6.43 $\times$ 10$^{10}$ & 9.18 $\times$ 10$^{10}$ & 269.2 & 5.12 $\times$ 10$^5$ & \textbf{5.86 $\times$ 10$^{10}$}\\
GM2 & 8.69 $\times$ 10$^{11}$ & 7.41 $\times$ 10$^{10}$ & 1.38 $\times$ 10$^{10}$ & X $\times$ 10$^{10}$ & 254.1 & X $\times$ 10$^5$ & \textbf{3.97 $\times$ 10$^{10}$}  \\ % This used to be GM7
GM3 & 7.76 $\times$ 10$^{11}$ & 6.36 $\times$ 10$^{10}$ & 1.04 $\times$ 10$^{10}$ & 6.35 $\times$ 10$^{10}$ & 241.7 & 4.14 $\times$ 10$^5$ & \textbf{2.45 $\times$ 10$^{10}$} \\ [1ex] % [1ex] adds vertical space; GM3 used to be GM4
\hline %inserts single line
\end{tabular}
...
%heading
\hline % inserts single horizontal line
P0 noBH & 8.36 $\times$ 10$^{11}$ & 7.0 $\times$ 10$^{10}$ & 7.37 $\times$ 10$^{10}$ & X $\times$ 10$^{10}$ & 262.8 & X $\times$ 10$^5$ & \textbf{7.34 $\times$ 10$^{10}$}\\ % inserting body of the table
GM1 noBH & 8.38 $\times$ 10$^{11}$ & 7.07 $\times$ 10$^{10}$ & 7.34 $\times$ 10$^{10}$ & X $\times$ 10$^{10}$ & 263.0 & X $\times$ 10$^5$ & \textbf{5.86 $\times$ 10$^{10}$}\\
GM2 noBH & 8.42 $\times$ 10$^{11}$ & 7.08 $\times$ 10$^{10}$ & 7.33 $\times$ 10$^{10}$ & X $\times$ 10$^{10}$ & 263.4 & X $\times$ 10$^5$ & \textbf{3.97 $\times$ 10$^{10}$}  \\ % This used to be GM7

GM3 noBH & 8.43 $\times$ 10$^{11}$ & 7.19 $\times$ 10$^{10}$ & 7.28 $\times$ 10$^{10}$ & X $\times$ 10$^{10}$ & 263.5 & X $\times$ 10$^5$ & \textbf{2.45 $\times$ 10$^{10}$} \\ [1ex] % [1ex] adds vertical space; GM3 used to be GM4
\hline %inserts single line
\vspace{5.111mm}
\end{tabular}
...
\begin{figure*}[h!]
\centerline{\resizebox{0.45\hsize}{!}{\includegraphics[angle=0]{P0_phasediagram_mass}}
\resizebox{0.45\hsize}{!}{\includegraphics[angle=0]{P0noBH_phasediagram_mass}}}

\centerline{\resizebox{0.45\hsize}{!}{\includegraphics[angle=0]{GM1_phasediagram_mass}}
\resizebox{0.45\hsize}{!}{\includegraphics[angle=0]{GM1noBH_phasediagram_mass}}}

\centerline{\resizebox{0.45\hsize}{!}{\includegraphics[angle=0]{GM2_phasediagram_mass}}
\resizebox{0.45\hsize}{!}{\includegraphics[angle=0]{GM2noBH_phasediagram_mass}}}

\centerline{\resizebox{0.45\hsize}{!}{\includegraphics[angle=0]{GM3_phasediagram_mass_test}}
\resizebox{0.45\hsize}{!}{\includegraphics[angle=0]{GM3noBH_phasediagram_mass}}}
...
\begin{figure*}[h!]
\centerline{\resizebox{0.35\hsize}{!}{\includegraphics[angle=0]{P0_phasediagram_mass}}
\resizebox{0.35\hsize}{!}{\includegraphics[angle=0]{P0_phasediagram_metallicity}}
\resizebox{0.35\hsize}{!}{\includegraphics[angle=0]{P0_phasediagram_Rkpc}}}

\centerline{\resizebox{0.35\hsize}{!}{\includegraphics[angle=0]{P0noBH_phasediagram_mass}}
\resizebox{0.35\hsize}{!}{\includegraphics[angle=0]{P0noBH_phasediagram_metallicity}}
\resizebox{0.35\hsize}{!}{\includegraphics[angle=0]{P0noBH_phasediagram_Rkpc}}}

\centerline{\resizebox{0.35\hsize}{!}{\includegraphics[angle=0]{GM2_phasediagram_mass}}
\resizebox{0.35\hsize}{!}{\includegraphics[angle=0]{GM2_phasediagram_metallicity}}
\resizebox{0.35\hsize}{!}{\includegraphics[angle=0]{GM2_phasediagram_Rkpc}}}
...
\begin{figure*}[ht!]
\centerline{\resizebox{0.48\hsize}{!}{\includegraphics[angle=0]{Novi_profile_ALLMWROM_plusobs}}
\resizebox{0.48\hsize}{!}{\includegraphics[angle=0]{Novi_profile_ALLMWROM_GMs_plusobs}}}
...
\begin{figure}[h!]
\centerline{\resizebox{0.98\hsize}{!}{\includegraphics[angle=0]{ALLGMs_plusnoBH_Novi_R}}}
...
% Then we remove the BH physics from all of our runs and re-simulate.  (goes in parameters section?)
...
\begin{figure*}
\centerline{\resizebox{0.45\hsize}{!}{\includegraphics[angle=0]{ALLGMs_plusnoBH_T_R}}
\resizebox{0.45\hsize}{!}{\includegraphics[angle=0]{ALLGMs_plusnoBH_rho_R}}}
\centerline{\resizebox{0.45\hsize}{!}{\includegraphics[angle=0]{ALLGMs_plusnoBH_totgasmass_R}}
\resizebox{0.45\hsize}{!}{\includegraphics[angle=0]{ALLGMs_plusnoBH_Z_R}}}
...
\begin{figure}[h!]
\centerline{\resizebox{1.05\hsize}{!}{\includegraphics[angle=0]{ALLGMs_DISKmetals_R_plusnoBHs}}}
...
% Result 1: OVI as a Tracer for Virial Temperature of the Halo

% CHANGA is well simulating NOVI column densities compared to COS-Halo
% This is true regardless of whether the galaxy quenches or is star forming.
...
% Evolution of the galaxy doesn’t matter as much as the virial temperature of the galaxy.
...
\begin{figure*}[ht!]
\centerline{\resizebox{0.95\hsize}{!}{\includegraphics[angle=0]{OPPEN_fig10_ROMgxys_plushighmass}}}
...
% Result 2: AGN as driver for metals in the CGM
...
% Without BH physics however, the NOVI column densities are much lower (comparable to other simulation attempts to measure NOVI)
...
% The metallicity of the CGM is significantly lower in the no BH case.
% Without an AGN, there is no mechanism to drive metals out from the center of the galaxy.
%The metallicity profile of the gas in the disk (Figure ) shows a build up of metals in the center of the galaxies without BH physics. Without the AGN, metals are not being propagated into the CGM but instead remaining locked in the gas of the disk.
% AGN plays an important role in populating the metals in the CGM (But not through heating which has been previously key idea!)
...
% Conclusions (2-4 Paragraphs)

% We have shown that the AGN in MW galaxies puts OVI in the CGM and OVI acts as a tracer for the virial temperature/mass of host halos.
